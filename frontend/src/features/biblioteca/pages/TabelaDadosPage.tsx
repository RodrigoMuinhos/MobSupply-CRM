  ...
  return (
    <div className="p-4" style={{ background: temaAtual.fundo, color: temaAtual.texto }}>
      ...
      <div className="text-sm mb-4">
        {[
          { cor: 'bg-blue-400', label: 'Não Conferido', status: 0 },
          { cor: 'bg-yellow-400', label: 'Revisar', status: 1 },
          { cor: 'bg-green-500', label: 'Ativos', status: 2 },
          { cor: 'bg-red-700', label: 'Inativos', status: 3 },
        ].map(({ cor, label, status }) => (
          <span key={status} className="inline-flex items-center mr-4">
            <span className={`w-3 h-3 rounded-full ${cor} mr-1`} />
            {label}: <strong>{contagemStatus[status] || 0}</strong>
          </span>
        ))}
      </div>
