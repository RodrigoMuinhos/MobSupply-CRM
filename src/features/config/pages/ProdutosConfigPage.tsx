...
const ProdutosConfigPage: React.FC = () => {
  ...
  const handleChange = (
    ...
    if (campo === 'quantidade') {
      ...
      item.quantidade = isNaN(qtd) ? 0 : qtd;
    } else if (campo === 'nome') {
      item.nome = valor;
  ...
  const handleBlur = (
    ...
    const valorDigitado = valoresInputs[produto.codigo]?.[campo] || '';
    const limpo = valorDigitado.replace(/[^\d,]/g, '').replace(',', '.');
    const numero = parseFloat(limpo);

    const copia = [...produtos];
    if (campo === 'preco') {
      copia[index].preco = isNaN(numero) ? 0 : parseFloat(numero.toFixed(2));
    } else {
      copia[index].valorVenda = isNaN(numero) ? '0' : numero.toFixed(2);
    }
    setProdutos(copia);
