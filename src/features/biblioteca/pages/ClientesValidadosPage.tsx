...
const ClientesValidadosPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const validos = Array.isArray(salvos)
      ? salvos.filter((c) => c.cnpj && Array.isArray(c.contatos))
      : [];
  ...
  const handleEditarContato = (cnpj: string | number, index: number, novo: string) => {
    const atualizados = clientes.map((c) => {
      if (c.cnpj === cnpj) {
        const novosContatos = [...c.contatos];
        novosContatos[index] = novo;
  ...
  const handleRemoverContato = (cnpj: string | number, index: number) => {
    const atualizados = clientes.map((c) => {
      if (c.cnpj === cnpj) {
        const novosContatos = c.contatos.filter((_, i) => i !== index);
        return { ...c, contatos: novosContatos };
      }
      return c;
    });
  ...
  const contagemPorCor = cores.reduce((acc, cor) => {
    const count = clientes.filter((c) => statusMap[String(c.cnpj)] === cor).length;
    acc[cor] = count;
    return acc;
